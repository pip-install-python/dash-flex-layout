...
Ported from pip-docs+ (the reference, item 16). One source of truth: the
...
def parse_changelog(path: Path = CHANGELOG_PATH) -> list[dict]:
    ...
    for line in path.read_text(encoding="utf-8").split("\n"):
        # ASCII hyphen, en dash or em dash between version and date; a bare
        # `## [Unreleased]` with no date at all. Every heading shape this
        # file uses:
        #   ## [2.0.0] - 2026-08-02
        #   ## [Unreleased]
...
def _code(text: str):
    """`code` spans inside one fragment."""
    out = []
...
def _inline(text: str):
    ...
    CONTAINING inline code renders its asterisks raw when code is split
    first, because the bold markers then sit in different fragments."""
    out = []
...
def _version_item(v: dict, is_current: bool):
    ...
    # `v` only when the label IS a version number: `## [Unreleased]` read as
    # "vUnreleased" before this fix. A date-labelled release keeps its date
    # as the label verbatim (never "vYYYY-MM-DD").
...
# The full machine record (item 18; leaflet's finding): a module-level
# LLMS_DOC alone leaves the package to discover the page with no `lastmod`,
# so /changelog entered the sitemap undated — and outside the control
# board's llms.txt toggle. Same two calls pages/markdown.py makes for every
# docs page; lastmod = the newest dated release heading.
