...
The read table dash-improve-my-llms >= 2.8.0 emits (``on_document_read``,
kept by ``lib.analytics_tracker.AnalyticsTracker.record_read``) is what
makes "does ClaudeBot actually get the corpus here?" answerable from the
host itself, before the hub folds anything: vendor × day, vendor → tier, and
...
Render-cost rule (fleet fact 18): plain tables, NO charts, mount once, no
interval callback. A 14 x 40 table of strings costs about a millisecond;
five charts cost ten seconds. The day picker is the only control.
"""
...
# Deliberately NOT registered in lib.page_tiers — same reasoning as the
# control board (pages/control_board.py): a `hidden` tier entry would flip
# `access.gating_configured()` on for EVERY fork, including all-public
# sites. This page's own fail-closed layout() is the gate; the package-level
# mark below keeps every machine surface silent about it.
try:
    ...
    return dmc.Group(
        [
            dmc.Paper(
                dmc.Stack(
                    [
                        dmc.Text(str(v), size="24px", fw=700,
                                 id=f"traffic-stat-{k.replace(' ', '-')}"),
...
def people_block(day: date):
    """PEOPLE — the v3 human numbers, in their own section (owner
    requirement 11, 2026-08-30). Humans never enter the read ledger: the
    package emits a read event for the crawler document only, so the
    tables below this section are crawlers, and "unidentified" there is
    the UA-less crawler lane, never a person."""
...
def day_picker(days, reads):
    ...
    DMC-first rule): bounded by the ledger's first and last day, with
    presets for the days someone actually asks about."""
    today = days[-1]
