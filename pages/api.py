"""/api — one prop table per component, generated from the package (item 16).

Registered ONLY when lib.constants.API_PACKAGES names something.
flexlayout-dash declares ``API_PACKAGES = ["flexlayout_dash"]`` — its own
package — so the page, the sidebar entry and the /api/llms.txt document
all exist for free from the installed component's metadata.json.
"""
...
def _docs_page_owns(path: str) -> bool:
    """A fork may serve /api as a DOCS page (a `.. kwargs::` table plus
    curated prose). Page modules load before pages/markdown.py, so the
    registry cannot be asked yet — read the docs' frontmatter directly and
    yield (item 18). Not the case on this fork today — docs/reference/
    reference.md registers at /reference, not /api — but the guard costs
    nothing and protects against a future docs page claiming the path."""
    import frontmatter
...
if API_PACKAGES and not _docs_page_owns("/api"):
    ...
    # The full machine record (item 18): visibility + tier so the control
    ...
    # and no extract — then the sitemap omits the tag, honestly; this fork
    # runs scripts/build_api_metadata.py so it is populated).
