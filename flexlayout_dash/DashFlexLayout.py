...
- modelAction (dict with strings as keys and values of type boolean | number | string | dict | list; optional):
    Imperative FlexLayout action applied to the LIVE model in place —
    works in both useStateForModel modes and never re-mounts sibling
    tabs (unlike replacing `model`, which is ignored when
    useStateForModel=True and re-mounts every tab otherwise).  Shape:
    {type, nonce, ...args}. `nonce` must change for the action to fire
    (guard against re-renders re-applying it). Supported types: -
    addNode: {json, toNodeId, location?:
    'top'|'bottom'|'left'|'right'|'center', index?, select?}   If
    json.id already exists in the model, the tab is selected instead
    of re-added. - deleteTab: {tabNodeId} - selectTab: {tabNodeId} -
    renameTab: {tabNodeId, text} - updateNodeAttributes: {nodeId,
    attributes} - adjustWeights: {nodeId, weights}.
...
- useStateForModel (boolean; default False):
    ...
    def __init__(
        ...
        modelAction: typing.Optional[typing.Dict[typing.Union[str, float, int], typing.Any]] = None,
        colorScheme: typing.Optional[Literal["light", "dark"]] = None,
        style: typing.Optional[typing.Any] = None,
        loading_state: typing.Optional["LoadingState"] = None,
        **kwargs
    ):
        self._prop_names = ['children', 'id', 'colorScheme', 'debugMode', 'font', 'headers', 'loading_state', 'model', 'modelAction', 'popoutURL', 'realtimeResize', 'style', 'supportsPopout', 'useStateForModel']
        self._valid_wildcard_attributes =            []
        self.available_properties = ['children', 'id', 'colorScheme', 'debugMode', 'font', 'headers', 'loading_state', 'model', 'modelAction', 'popoutURL', 'realtimeResize', 'style', 'supportsPopout', 'useStateForModel']
