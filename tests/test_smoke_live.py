...
def wired(smoke, client, monkeypatch):
    ...
    def fetch(url, user_agent=smoke.BROWSER_UA, accept=None,
        ...
        # `retries`/`timeout` are the post-1.6.29 signature and are ignored
        # here on purpose — the ladder is exercised against a scripted
        # transport below, not against the test client. Accepting them is
        # the port's second half: a stub written for the old
        # `(url, user_agent, accept)` shape TypeErrors inside wake(), which
        # is how the 1.6.28 fan-out went red on 7 of 12 forks before a
        # single check ran (SYNC-1.6.22-1.6.29 item 6).
    ...
    # The wake loop is a live-host concern (Render cold starts, and the few
    # seconds of 502 while a container with a disk swaps); these tests are
    # about the checks. It gets its own tests below, against a scripted
...
def test_smoke_script_detects_a_foreign_canonical(wired, smoke, monkeypatch, capsys):
    ...
    def rehosted(url, user_agent=smoke.BROWSER_UA, accept=None):
        ...
        return status, body.replace(
            'rel="canonical" href="https://flexlayout.2plot.dev',
...
# ---------------------------------------------------------------------------
# Transient immunity — the fleet-wide CD flake (SYNC-1.6.22-1.6.29 item 6).
#
# The battery runs against Render tiers where a cold start or a dropped
# connection is routine, and against a starter service with a disk, which
# answers 502 for a few seconds on every deploy swap while it restarts. A
# single-shot fetch turned those into `FAIL canonical on /<page>` — a check
# that never actually ran — and CD went red on healthy sites (measured on
# dash-flows-upgraded: two runs minutes apart, same host, opposite verdicts).
# These tests pin the two defenses: fetch retries transports and 5xx (and
# ONLY those), and main() wakes the host before asserting anything about it.
# ---------------------------------------------------------------------------
...
class _FakeTime:
    ...
    the app under test runs background threads (the analytics flusher, the
    satellite presence poster) that also call time.sleep, and a global no-op
    would turn them into busy-loops for the duration of the test.
    """
...
def test_a_cold_host_wakes_and_the_probe_requires_ok_true(smoke, monkeypatch, capsys):
    """A 502 mid-swap (or Render's loading page) greets probe one; ok:true
    ends it.
...
def test_wake_survives_a_legacy_fetch_stub(smoke, monkeypatch, capsys):
    ...
    and went red on 7 of 12 forks before a single check ran. wake falls back
    to a bare `fetch(url)` when the stub rejects its kwargs, so a template
    copy landing ahead of a fork's stub update degrades to that fork's own
    honest check results instead of a suite-wide crash. THIS fork's stub was
    updated in the same touch as the port; the fallback is kept armed anyway,
    because the next copy to arrive is the one nobody is watching.
    """
...
def test_a_host_that_never_wakes_is_one_failure_not_a_cascade(
    ...
    """Fifty per-check failures against a dead host all say the same thing."""
    monkeypatch.setattr(smoke, "time", _FakeTime())
