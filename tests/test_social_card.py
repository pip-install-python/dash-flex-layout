...
Copied from the boilerplate (the network template); only the constants it
imports and the manifest's foreign-brand strings are per-site.

Both things tested here fail silently and fail OUTSIDE the app, which is why
they need tests rather than a look at the page — nobody sees their own
unfurls, and no browser explains why it declined to offer an install.
...
(per page, from `register_page(image_url=...)`); `og:site_name`, the
...
is why deleting index.html would silently kill every unfurl — and this repo
DID delete it once (2026-07-31) on exactly that misreading, which is what
`test_the_index_template_is_still_wired_in` now guards.
"""
...
def test_the_image_is_hosted_off_the_app(client):
    ...
    That the URL RESOLVES is deliberately not checked here (it would make the
    suite depend on Cloudflare being up). `scripts/smoke_live.py` fetches the
    real file after every deploy and checks its actual pixels against the
    constants — which also catches the CDN object being replaced with
    something a different shape, something no offline test can see.
    """
...
def test_no_meta_tag_dash_emits_is_also_declared_statically(client):
    """The rule the OG block in index.html is built on.
...
def test_the_manifest_describes_THIS_site(client):
    """It shipped naming the HUB — "2plot.dev — Dash Components Documentation"
    — copied in with the favicon set. An installed app takes its home-screen
    label from `short_name`, so this is the one place a wrong string becomes a
    permanent icon on someone's phone.
    """
...
def test_every_asset_the_template_references_resolves(client):
    ...
    The deploy builds from git, so a template pointing at files that exist on
    disk but sit untracked 404s in production while every local boot looks
    perfect. A checkout is what CI tests, so this fails there the moment a
    referenced asset is not committed.
    """
    html = _visible(client.get("/").text)
...
def test_the_index_template_is_still_wired_in(app_module):
    """`templates/index.html` looks removable and is not — it WAS removed once.
    ...
    template kills the og auxiliaries, the icons and the manifest at once.
    """
