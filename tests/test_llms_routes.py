...
def test_page_llms_txt_carries_the_page_prose(client):
    body = client.get("/reference/llms.txt").text
    assert "Component Reference" in body
    assert "DashFlexLayout" in body, "page prose did not reach /<page>/llms.txt"


def test_no_unexpanded_directive_reaches_any_page_document(client, page_paths):
    """Whatever the docs use, an agent must never receive a raw directive.

    The audience for /<page>/llms.txt is someone pasting it into a chat
    window; `.. source::docs/x/example.py` is useless to them.
    """
    leaks = [p for p in docs_paths(page_paths)
             if ".. source::" in client.get(
                 "/llms.txt" if p == "/" else f"{p.rstrip('/')}/llms.txt").text]
    assert leaks == [], f"unexpanded directives leaked into: {leaks}"


def test_source_directives_inline_the_referenced_file(tmp_path, monkeypatch):
    """The expander itself, exercised directly.

    No page in THIS repo currently writes a `.. source::` directive — every
    example is a live `.. exec::` instead — so the round trip above can only
    prove the absence of leaks. The machinery is still wired into
    pages/markdown.py and the moment a doc uses it the output has to be the
    file's contents, so it is tested where it lives rather than left to a
    page that may never exist.
    """
    import pages.markdown as md

    target = tmp_path / "snippet.py"
    target.write_text("def apply(app_url: str):\n    return app_url\n")

    expanded = md._expand_source_directives(f".. source::{target}")
    assert ".. source::" not in expanded, "the directive survived expansion"
    assert "def apply(app_url: str)" in expanded, "the file was not inlined"
    assert "```python" in expanded, "the inlined file lost its language fence"


def test_a_missing_source_file_degrades_to_a_comment(tmp_path):
    """A typo'd path must not take a page's whole document down with it."""
    import pages.markdown as md

    expanded = md._expand_source_directives(".. source::docs/nope/missing.py")
    assert "File not found" in expanded
    assert ".. source::" not in expanded
...
def test_sitemap_lists_every_page_on_this_host(client, page_paths):
    ...
    missing = {p.rstrip("/") or "/" for p in docs_paths(page_paths)} - listed
    assert not missing, f"pages absent from sitemap.xml: {sorted(missing)}"

    # ...and the owner surface is absent, which is the other half of the same
    # contract: `mark_hidden` in pages/control_board.py keeps /admin out of
    # the sitemap. Gated DOCUMENTATION pages stay listed — hiding who may read
    # a page is not a reason to hide that it exists — but the control board is
    # not documentation.
    assert not (ADMIN_PATHS & listed), (
        f"an owner-only surface is advertised in sitemap.xml: "
        f"{sorted(ADMIN_PATHS & listed)}"
    )
...
# Any page document does; this one is the most prose-heavy on the site, so a
# chrome leak has the most text to hide in.
PAGE_DOC = "/reference/llms.txt"
...
def test_a_page_may_document_the_viewer_without_tripping_the_check():
    """Prose that NAMES the viewer's classes is content, not chrome.

    The boilerplate proves this with a page that documents the viewer; no page
    here does, so the distinction is pinned against the regex itself. It
    matters because the obvious "fix" when chrome leaks is to make the check
    substring-based — which would then fail any page that writes about the
    viewer, and teach people to stop writing about it.
    """
    prose = "Browsers get a `dv-banner` header and the `mk-wordmark` logo."
    assert not CHROME.search(prose), "a mention of the class must not read as chrome"
    assert CHROME.search('<div class="dv-banner">'), "real chrome must still match"
