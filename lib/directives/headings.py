...
Item 16 (2026-08-30) also patches inline image rendering here: markdown2dash
defines no `image` renderer, so mistune's HTML fallback runs and raises on
the DMC child list this renderer builds. Rendered as a plain `<img>`.
"""
...
def patch_renderer() -> None:
    ...
    Idempotent — importing this module twice is harmless. This fork's own
    ``lib/renderer.py`` subclasses ``DashRenderer`` too (the bold/italic <p>
    fix); since that subclass overrides neither ``heading`` nor ``image``,
    it inherits whatever this function patches onto the base class, and
    both fixes apply together.
    """
    ...
    # mistune's HTML fallback runs and raises on the DMC child list. Rendered
    # as a plain <img> with the alt text — a decorative shield or a hero
    # image, never a layout component.
