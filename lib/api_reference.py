"""Component prop tables from an installed Dash component package (item 16).

Three sources, in order (item 18 — leaflet's and modelviewer's findings):
...
1. ``metadata.json`` next to the package's ``__init__`` — react-docgen
   ...
   multi-megabyte gitignored build artifact excluded from the wheel
   (leaflet), so /api passes locally and is EMPTY on the host. THIS FORK
   commits its own metadata.json (not gitignored — confirmed via
   `git ls-files`), so mechanism 1 is what actually serves /api here; see
   DIVERGENCES.md for the ledger of why it stays that way.
2. ``api_metadata.json`` next to the package — the committed extract
   ...
   Present on this fork too (mechanism 1 still wins for the table
   CONTENT; this mechanism only supplies /api's lastmod stamp — see
   `slim_generated_on`).
