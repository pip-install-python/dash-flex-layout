...
def app_key() -> str:
    ...
    Default is "flexlayout", this app's own key in the hub directory.
    The template this file was copied from defaults to "boilerplate" and
    warns that a clone MUST override it or its reports overwrite another
    app's rows — so the default is changed here rather than left to an
    environment variable that a local run or a misconfigured deploy could
    omit. SATELLITE_APP_KEY still wins if set (render.yaml sets it too).
    """
    return os.getenv("SATELLITE_APP_KEY") or "flexlayout"
...
def post_rollup(payload: dict, secret: str | None = None, timeout: float = 10.0):
    ...
    try:
        # Internal-traffic contract (lib/constants.INTERNAL_UA): a signed
        # rollup is network machinery — without the token the hub's own
        # tracker counts every report as a bot hit on 2plot.ai.
