...
Rollup v4 adds the VENDOR dimension, additively, from the ledger's second
table — the ``reads`` rows dash-improve-my-llms >= 2.8.0 emits for every
...
- ``vendors[]`` — one row per ``(vendor key, verified, policy)`` for the
  ...
  a future pricing decision would need, and dropping it would hide exactly
  that.
- ``reads`` — the day's read total, ``sum(vendors[].hits)``: the
  reconciliation number against a host's bot accounting.

Both keys are present ONLY when the day has reads. Every v3 key stays
...
def load_reads(path=None):
    ...
    A ledger with no ``reads`` key (written before the 2.8.0 floor, or by a
    host below it) reads as empty. No ``_SKIP`` filtering: the package only
    emits for documents it served, which are exactly the surfaces the
    visits reader skips.
    """
    import json
...
def daily_rollup(app: str, day: date | None = None, visits=None,
    ...
    if day_reads:
        ...
        # below the 2.8.0 floor and a quiet day are indistinguishable by
        # absence — which is the truthful state of both.
