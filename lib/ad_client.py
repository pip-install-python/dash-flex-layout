...
app (dash-email, dash-mui-scheduler, dash-flows, dash-leaflet2, ...).
This copy serves the flexlayout-dash docs site at https://flexlayout.2plot.dev.
...
# The SHORT directory key (hub 2.2.0 rule) — matches SATELLITE_APP_KEY and the
# bulletin app_id, and is what the hub's /admin/ad-analytics rows key on. The
# hub folds the legacy "flexlayout-dash" spelling at ingest, but new builds
# must send the canonical id.
APP_ID = os.environ.get("AD_APP_ID", "flexlayout")

_TIMEOUT = 2          # seconds per fetch — never stall a page view longer
_COOLDOWN = 60        # seconds to skip fetches after a failure
_session = requests.Session()
# Internal-traffic contract (lib/constants.INTERNAL_UA): this fetch fires on
# every docs page view and lands on 2plot.dev, whose tracker classifies bare
# `python-requests` as a bot — so without the token, this satellite's readers
# inflate the hub's bot_hits. Clicks are NOT tagged: they arrive from the
# reader's own browser, and a click IS a real person.
from lib.constants import internal_ua as _internal_ua

_session.headers["User-Agent"] = _internal_ua("ad-client")
