...
(the thing under test); this project's own environment supplies the docs-site
...
Env:
    FLD_EXTRA_SITE   donor site-packages directory
    FLD_SMOKE_ARGS   arguments to forward to smoke_test.py, newline-separated
"""
import os
import runpy
import sys
...
extra = os.environ.get("FLD_EXTRA_SITE")
...
sys.argv = ["smoke_test.py"] + [
    a for a in (os.environ.get("FLD_SMOKE_ARGS") or "").split("\n") if a
]
